...
import { UploadCloud, PlayCircle, Activity, Scale, FileSpreadsheet, BotMessageSquare, Sparkles, Shield, ChevronRight, Settings2, FileBarChart2, BookOpenText, Download } from "lucide-react";
import { cn } from "@/lib/utils";
import { toast } from "@/hooks/use-toast";

export default function ConsolePage() {
  // RBAC
  const [role, setRole] = useState<string>(localStorage.getItem("pc_role") || "admin");
    ...
    const id = setInterval(() => {
      setProgress((p) => {
        ...
        if (v >= 100) {
          clearInterval(id);
          setTraining(false);
          toast({ title: "Training completed", description: `${modelType} metrics available` });
        }
        return v;
      });
    }, 600);
  ...
  const runEDA = () => {
    if (!fileName) {
      toast({ title: "No file selected", description: "Choose a dataset first" });
      return;
    }
    setEdaReady((s) => !s);
  ...
  const validateSchema = () => {
    toast({ title: "Schema valid", description: "Columns and target look good" });
  };
  ...
  const startTraining = () => {
    ...
    setProgress(0);
    setTraining(true);
    toast({ title: "Training started", description: `${modelType} • depth=${depth}, lr=${lr}, n=${estimators}` });
  };
        ...
        <div className="flex gap-2">
          <Dialog open={openRBAC} onOpenChange={setOpenRBAC}>
            <DialogTrigger asChild>
              <Button variant="secondary"><Shield className="mr-2 size-4"/>RBAC: {role}</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Switch role</DialogTitle>
                <DialogDescription>Demo-only, applies to UI visibility.</DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter>
                <Button onClick={()=>{ setOpenRBAC(false); toast({ title: "Role updated", description: role });}}>Apply</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
          ...
          <Dialog open={openNewJob} onOpenChange={setOpenNewJob}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>Choose model type and basic parameters.</DialogDescription>
              </DialogHeader>
              <div className="grid gap-3">
                ...
                <div className="grid md:grid-cols-3 gap-3">
                  <div className="text-sm"><Label>Learning rate</Label><Input value={lr} onChange={(e)=>setLr(e.target.value)} /></div>
                  <div className="text-sm"><Label>Estimators</Label><Input value={estimators} onChange={(e)=>setEstimators(e.target.value)} /></div>
                  <div className="text-sm"><Label>Max depth</Label><Input value={depth} onChange={(e)=>setDepth(e.target.value)} /></div>
                </div>
              </div>
              <DialogFooter>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-4 flex gap-2 flex-wrap">
              <Button className="group" onClick={runEDA}><Activity className="mr-2 size-4"/>Run EDA<ChevronRight className="ml-1 size-4 transition-transform group-hover:translate-x-0.5"/></Button>
              <Button variant="outline" onClick={validateSchema}><BotMessageSquare className="mr-2 size-4"/>Validate schema</Button>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            {training ? (
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm"><span>Training…</span><span>{Math.round(progress)}%</span></div>
                <Progress value={progress} />
              </div>
            ) : (
              <div className="flex gap-2 flex-wrap">
                <Button size="sm" onClick={startTraining}><PlayCircle className="size-4 mr-1"/>Start training</Button>
                <Dialog open={openParams} onOpenChange={setOpenParams}>
                  <DialogTrigger asChild>
                    <Button size="sm" variant="outline"><Settings2 className="mr-1 size-4"/>Configure params</Button>
                  </DialogTrigger>
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>Training parameters</DialogTitle>
                      <DialogDescription>Adjust hyperparameters.</DialogDescription>
                    </DialogHeader>
                    <div className="grid md:grid-cols-3 gap-3">
                      <div className="text-sm"><Label>Learning rate</Label><Input value={lr} onChange={(e)=>setLr(e.target.value)} /></div>
                      <div className="text-sm"><Label>Estimators</Label><Input value={estimators} onChange={(e)=>setEstimators(e.target.value)} /></div>
          ...
          <CardContent>
            <div className="flex gap-2 flex-wrap">
              <Dialog open={openMetrics} onOpenChange={setOpenMetrics}>
                <DialogTrigger asChild>
                  <Button size="sm" variant="outline"><FileBarChart2 className="mr-1 size-4"/>Open metrics</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader><DialogTitle>Model metrics</DialogTitle></DialogHeader>
                  <div className="h-52">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={Array.from({length:10}).map((_,i)=>({t:i, auc:0.7+Math.sin(i/2)/10}))}>
                        <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--muted-foreground)/0.2)"/>
                        <XAxis dataKey="t" />
                        <YAxis domain={[0.5,1]} />
                        <Tooltip />
                        <Line dataKey="auc" stroke="hsl(var(--primary))" dot={false} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </DialogContent>
              ...
              <Dialog open={openExplain} onOpenChange={setOpenExplain}>
                <DialogTrigger asChild>
                  <Button size="sm"><BookOpenText className="mr-1 size-4"/>Global explainability</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader><DialogTitle>SHAP feature importance</DialogTitle></DialogHeader>
                  <div className="h-60">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={[
                        {f:"age", v: 0.28}, {f:"blood_pressure", v: 0.22}, {f:"cholesterol", v:0.17}, {f:"bmi", v:0.12}, {f:"smoker", v:0.08}
                      ]}>
                        <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--muted-foreground)/0.2)"/>
                        <XAxis dataKey="f" />
                        <YAxis />
                        <Tooltip />
                        <Bar dataKey="v" fill="hsl(174 62% 45%)" radius={[4,4,0,0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </DialogContent>
          ...
          <CardContent>
            <div className="flex gap-2 flex-wrap">
              <Dialog open={openLogs} onOpenChange={setOpenLogs}>
                ...
                <DialogContent>
                  <DialogHeader><DialogTitle>Audit log</DialogTitle></DialogHeader>
                  <ul className="text-sm space-y-2 max-h-64 overflow-auto">
                    {["dataset.upload","eda.run","train.start","train.complete","metrics.view"].map((e,i)=> (
                      <li key={i} className="flex items-center justify-between border rounded-md px-3 py-2">
                        <span>{e}</span>
                        <span className="text-muted-foreground">{new Date(Date.now()-i*3600_000).toLocaleString()}</span>
                      </li>
                    ))}
                  </ul>
                </DialogContent>
              </Dialog>
              <Button size="sm" onClick={exportAudit}><Download className="mr-1 size-4"/>Export</Button>
            </div>
          </CardContent>
        </Card>
      </div>
